          ...
          data.map((row, i) => (
            <div key={i} className="quiz-row">
              <div className="col student-col">
                {row.profilePic ? (
                  <img src={row.profilePic} alt={row.student} className="avatar-circle" />
                ) : (
                  <div className="avatar-circle avatar-fallback">
                    {row.student.slice(0, 2).toUpperCase()}
                  </div>
                )}
                {row.student}
              </div>
              ...
              <div className="col status-col">
                <button
                  className={`status-btn ${row.status.toLowerCase().includes('complete') ? 'completed' :
