...
- Quizzes have difficulty levels
...
exports.chat = async (req, res) => {
    try {
        ...
        const messages = buildConversationMessages(message, conversationHistory, userContext); // Added userContext to call
...
exports.chatNonStreaming = async (req, res) => {
    try {
...
exports.generateWrongAnswers = async (req, res) => {
    try {
...
Return as JSON array only.`;
                    ...
                    return {
                        ...
                        wrongAnswers: wrongAnswers.slice(0, 4)
                    };
                } catch (error) {
...
// Generate complete quiz with AI
exports.generateQuiz = async (req, res) => {
    try {
        ...
        if (!category || !course || !topic || !lesson) {
            return res.status(400).json({ success: false, error: 'Category, course, topic, and lesson are required' });
        }
        ...
        if (!numQuestions || numQuestions < 3 || numQuestions > 10) {
            return res.status(400).json({ success: false, error: 'Number of questions must be between 3 and 10' });
        }
        if (!numAnswers || numAnswers < 3 || numAnswers > 5) {
            return res.status(400).json({ success: false, error: 'Number of answers must be between 3 and 5' });
        }
        if (!openai || !process.env.OPENAI_API_KEY) {
            return res.status(503).json({ success: false, error: 'AI service is not configured. Please contact support.' });
        }
...
Return ONLY the JSON object, nothing else.`;
        ...
        res.status(500).json({ success: false, error: error.message || 'Failed to generate quiz. Please try again.' });
    }
};
...
exports.getDailyTip = async (req, res) => {
    if (!openai) {
        return res.status(503).json({ success: false, error: 'AI service is not available. Please configure OPENAI_API_KEY.' });
    }
